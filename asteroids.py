...
ASTEROID_COLOR = (14, 7, 0)   #(101, 67, 33) = brown
SHIP_SIZE = 10
BULLET_SIZE = 4
ASTEROID_SIZE = 80
SHIP_SPEED = 2
...
#variable to track the last points_threshold crossed
last_threshold = 0
...
def reset_game():
    ...
    global lives, game_over, bullets, asteroids, points, last_threshold, current_asteroid_speed
    ...
    points = 0
    last_threshold = 0
    current_asteroid_speed = ASTEROID_SPEED

def check_difficulty(asteroids, points, last_threshold):
    current_threshold = points // 50 #determines the current threshold based on points
    new_asteroids = []

    if current_threshold > last_threshold: #checks if a new threshold is crossed
        for asteroid in asteroids:
            asteroid_x, asteroid_y, angle, speed = asteroid
            new_speed = speed + 1
            new_asteroids.append((asteroid_x, asteroid_y, angle, new_speed))
        last_threshold = current_threshold #updates the last threshold
    else:
        new_asteroids = asteroids 

    return new_asteroids, last_threshold 
...
while running:
    ...
    asteroids, last_threshold = check_difficulty(asteroids, points, last_threshold)
